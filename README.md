...
- `TablaService` ahora toma el `user_id` para el log de auditoría directamente del JWT verificado (antes venía de un header `X-User-Id` que armaba el cliente y no se validaba). El endpoint `POST /historial/` (usado directo desde `historial-cambios.service.ts` en el frontend) todavía acepta un `user_id` en el body sin verificarlo contra el token — queda pendiente aplicar el mismo criterio ahí.
